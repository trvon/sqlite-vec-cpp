...
inline Result<void> register_all_functions(sqlite3* db) {
    ...
    rc = sqlite3_create_function_v2(db, "vec_distance_hamming", 2, kVecReadFlags, nullptr,
    ...
    rc = sqlite3_create_function_v2(db, "vec_length", 1, kVecReadFlags, nullptr, vec_length,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_length", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_type", 1, kVecReadFlags, nullptr, vec_type, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
    ...
    rc = sqlite3_create_function_v2(db, "vec_f32_simple", 1, kScalarFlags, nullptr, vec_f32_simple,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
    ...
    rc = sqlite3_create_function_v2(db, "vec_int8", 1, kVecResultFlags, nullptr, vec_int8, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
    ...
    rc = sqlite3_create_function_v2(db, "vec_bit", 1, kVecResultFlags, nullptr, vec_bit, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_bit", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_to_json", 1, kVecReadFlags, nullptr, vec_to_json,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_to_json", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_add", 2, kVecResultFlags, nullptr, vec_add, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_add", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_sub", 2, kVecResultFlags, nullptr, vec_sub, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_sub", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_normalize", 1, kVecResultFlags, nullptr, vec_normalize,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_normalize", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_slice", 3, kVecResultFlags, nullptr, vec_slice,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_slice", rc));
    }

    // Register enhanced functions (C++20/23 specific features)
    rc = sqlite3_create_function_v2(db, "vec_dot", 2, kVecReadFlags, nullptr, vec_dot, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_dot", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_magnitude", 1, kVecReadFlags, nullptr, vec_magnitude,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_magnitude", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_scale", 2, kVecResultFlags, nullptr, vec_scale,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_scale", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_mean", 1, kVecReadFlags, nullptr, vec_mean, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_mean", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_std", 1, kVecReadFlags, nullptr, vec_std, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_std", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_min", 1, kVecReadFlags, nullptr, vec_min, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_min", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_max", 1, kVecReadFlags, nullptr, vec_max, nullptr,
                                    nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_max", rc));
    }

    rc = sqlite3_create_function_v2(db, "vec_clamp", 3, kVecResultFlags, nullptr, vec_clamp,
                                    nullptr, nullptr, nullptr);
    if (rc != SQLITE_OK) {
        return err<void>(Error::sqlite_error("Failed to register vec_clamp", rc));
    }
